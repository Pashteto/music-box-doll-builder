...
import (
	...
	"dollbuilder/internal/module"
	"dollbuilder/internal/repository"
	"dollbuilder/internal/service"
	wsmod "dollbuilder/internal/websocket"
	"dollbuilder/pkg/logger"
	"dollbuilder/pkg/version"
)
...
func (app *App) registerModules() error {
	...
	if app.config.HTTP != nil && app.config.HTTP.Enabled {
		...
		httpModule := httpmod.NewModule(app.config.HTTP, app.config.Auth, app.config.Stripe, app.svc, grpcClientModule)
